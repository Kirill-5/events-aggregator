...
async def metrics(
    ...
    events_count, created_count, cancelled_count = await asyncio.gather(
        event_repo.count(),
        outbox_repo.count_by_event_type("ticket_purchased"),
        outbox_repo.count_by_event_type("ticket_cancelled"),
    )
