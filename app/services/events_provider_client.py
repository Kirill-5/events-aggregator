...
class EventsProviderClient:
    ...
    async def _tracked_request(self, endpoint_label: str, method: str, url: str, **kwargs) -> httpx.Response:
        ...
        try:
            ...
            events_provider_requests_total.labels(endpoint=endpoint_label, status=status).inc()
            events_provider_request_duration_seconds.labels(endpoint=endpoint_label).observe(duration)

    async def events(self, cursor: Optional[str] = None, changed_at: Optional[str] = None) -> Dict[str, Any]:
        headers = {"x-api-key": self.api_key}

        if cursor:
            response = await self._tracked_request("/events", "GET", cursor, headers=headers)
        else:
            url = urljoin(self.base_url + "/", "api/events/")
            params = {}
            if changed_at:
                params["changed_at"] = changed_at
            response = await self._tracked_request("/events", "GET", url, params=params, headers=headers)
